...
import { CONFIG, SETUPS, fmtR, median } from '../lib/protocol.js';
...
const { bars: recent } = await market.dailyBars(assets.map(a => a.symbol), market.addDays(through + 'T00:00:00Z', -40), through, { onProgress: console.log });
...
const ranked = Object.entries(recent).filter(([, b]) => b.length >= 15 && b.at(-1).c >= CONFIG.minPrice).map(([s, b]) => [s, median(b.slice(-20).map(x => x.c * x.v))]).filter(([, dv]) => dv >= CONFIG.minDollarAdv).sort((a, b) => b[1] - a[1]).slice(0, N).map(([s]) => s);
...
const { cells, counts } = validateUniverse(bars, spy, folds, CONFIG, { onProgress: console.log });
const validation = { generatedAt: now.toISOString(), dataThrough: through, feed, folds, universe: `${counts.symbols} most liquid US stocks today (survivorship-biased)`, signals: counts.signals, config: CONFIG, cells };
await writeJson('data/validation.json', validation);

const lines = [`🧪 MONTHLY VALIDATION · data through ${through} · ${counts.symbols} stocks · ${counts.signals.toLocaleString()} out-of-sample trades`, `Test years: ${folds.map(f => `${f.from}→${f.to}`).join(' and ')}. Costs included. A cell goes live only if BOTH years clear ${fmtR(CONFIG.cellMinAvgR)} with ≥${CONFIG.cellMinTrades} trades and PF ≥ ${CONFIG.cellMinPF}.`];
