// Universe-level walk-forward validation. Nothing is fitted: the setups are fixed in
...
import { CONFIG, SETUPS, prepare, attachMomentumRanks, momentumUniverse, simulateTrade, seriesCharacter, characterFits, judgeCell, tradeStats, rollingMean, yangZhang } from './protocol.js';
...
export function validateUniverse(barsBySymbol, spy, folds, cfg = CONFIG, { onProgress = () => {} } = {}) {
  ...
  const syms = Object.keys(barsBySymbol).filter(sym => barsBySymbol[sym].length >= cfg.momMinBars);
  const preparedAll = Object.fromEntries(syms.map(sym => [sym, prepare(barsBySymbol[sym], cfg)]));
  attachMomentumRanks(momentumUniverse(preparedAll, cfg));
  syms.forEach((sym, n) => {
    const bars = barsBySymbol[sym];
    counts.symbols++;
    const s = preparedAll[sym], chCache = new Map();
    // Re-measured every 21 sessions, as of the block's FIRST bar, so no setup sees bars past i.
    const character = i => { const k = Math.floor(i / 21), at = k * 21; if (!chCache.has(k)) chCache.set(k, seriesCharacter(s.c.slice(Math.max(0, at - 755), at + 1), cfg)); return chCache.get(k); };
  ...
  for (const [key, labels] of Object.entries(cells)) for (const [label, fr] of Object.entries(labels)) {
    ...
    (result[key] ??= {})[label] = { pass: j.pass, tier: j.tier, fails: j.fails, pooled: j.pooled, folds: foldStats.map(({ rs, ...x }) => x) };
  }
