...
import { CONFIG, SETUPS, prepare, attachMomentumRanks, simulateTrade, seriesCharacter, characterFits, judgeCell, tradeStats, rollingMean, yangZhang } from './protocol.js';
...
export function validateUniverse(barsBySymbol, spy, folds, cfg = CONFIG, { onProgress = () => {} } = {}) {
  ...
  const syms = Object.keys(barsBySymbol).filter(sym => barsBySymbol[sym].length >= 760);
  const preparedAll = Object.fromEntries(syms.map(sym => [sym, prepare(barsBySymbol[sym], cfg)]));
  attachMomentumRanks(preparedAll);
  syms.forEach((sym, n) => {
