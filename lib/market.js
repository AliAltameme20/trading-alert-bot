...
export async function dailyBars(symbols, start, end, { feed = 'sip', onProgress = () => {} } = {}) {
  const out = {}; let used = feed;
  for (let i = 0; i < symbols.length; i += 200) {
    const chunk = symbols.slice(i, i + 200); let token, seen = new Set();
    for (let page = 0; page < 500; page++) {
      const p = { symbols: chunk.join(','), timeframe: '1Day', feed: used, adjustment: 'all', start: isoDay(start), end: endStamp(end), sort: 'asc', limit: '10000' };
...
export async function earningsWindow(dates, { fetchImpl = fetch } = {}) {
  const bySymbol = new Map(), sources = [];
  const add = (sym, date) => { sym = String(sym || '').toUpperCase().replace('/', '.'); if (!sym || !date) return; const cur = bySymbol.get(sym); if (!cur || date < cur) bySymbol.set(sym, date); };
  ...
  if (key) {
    try {
      ...
      for (const e of d.earningsCalendar) add(e.symbol, e.date);
  ...
  for (const date of dates) {
    try {
      ...
      for (const row of rows || []) add(row.symbol, date);
