...
import { CONFIG, SETUPS, marketRegime, readSymbol, sessionsUntil, fmtR, median, configForFeed, prepare, attachMomentumRanks, momentumUniverse, cellStatus } from './protocol.js';
...
// The exit that was validated for this plan — never a generic default.
export function exitRule(p) {
  if (p.exitAboveSma) return `Sell at the close on the first day it closes above its ${p.exitAboveSma}-day avg (market-on-close order), or at the close of session ${p.maxSessions}`;
  if (p.target) return `Target ${money(p.target)} · Max ${p.maxSessions} sessions`;
  return `No target: hold, and sell at the close of session ${p.maxSessions} unless stopped`;
}
...
export function formatDaily(out, { test = false } = {}) {
  ...
  for (const { rec, read, name, best } of out.signals) {
    ...
    L.push([
      `${rec.kind === 'paused' ? '[PAUSED — do not trade] ' : ''}${best.tier === 'probation' ? '[PROBATION · reduced size] ' : ''}BUY LIMIT ${rec.symbol} @ ${money(p.limit)} — day order, skip if unfilled`,
      `${name || ''}`.trim(),
      `Setup: ${best.label} · series ${read.character.label}${Number.isFinite(z) ? ` (z* ${z.toFixed(2)})` : ''}`,
      `Stop ${money(p.stop)} (1R = ${money(p.riskPerShare)}/sh) · ${exitRule(p)}`,
      ...
      `Edge: ${c.pooled.n.toLocaleString()} out-of-sample trades in this setup×series cell averaged ${fmtR(c.pooled.avgR)} after costs (PF ${c.pooled.pf.toFixed(2)}, t ${c.pooled.t.toFixed(1)}), ` + (best.tier === 'probation' ? `positive in both test years but under the full ${fmtR(CONFIG.cellMinAvgR)}/yr bar, so risk is cut to ${Math.round(CONFIG.probationRiskScale * 100)}%.` : `above ${fmtR(CONFIG.cellMinAvgR)} in both test years.`),
