...
export function readSymbol({ symbol, bars, spy, regime, validation, cfg = CONFIG, equity, riskPct, now = new Date(), prepared }) {
  ...
  const riskDollars = equity * riskPct * regime.riskScale * tierScale;
  ...
  r.sizing = { riskPct: riskPct * regime.riskScale * tierScale, riskDollars, shares, notional: shares * plan.limit };
