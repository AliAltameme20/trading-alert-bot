...
export const CONFIG = {
  ...
  maxOrderPctAdv: 0.01,
  // Hard sizing ceilings: they hold whatever RISK_PCT is set to. [A] 1% of the account per trade is
  // the most the measured edge supports at medium risk (Monte Carlo, Sept 26, 2026: typical worst
  // drop ~20% over 5 years); 25% per position keeps 4 open trades within the account.
  maxTradeRisk: 0.01,
  maxPositionPct: 0.25,
  // 01 regime (SPY)
  ...
  // Probation tier [A]: a cell that misses the full bar but is positive after costs in EVERY fold,
  // with pooled t ≥ 3 (the multiple-testing bar for ~20 cells re-measured monthly; Harvey, Liu &
  // Zhu 2016), trades at reduced size and is labelled as such. Set probationRiskScale: 0 to disable.
  probationMinAvgR: 0,         // every fold strictly above this, net of costs
  probationMinPF: 1.05,
  probationMinT: 3.0,
  probationRiskScale: 0.5,
  validationMaxAgeDays: 45,
  // 09 events
  earningsBlockSessions: 5,
  earningsUnknownBlocks: true, // fail closed: no verified date → no signal
  // Cross-sectional momentum is ranked inside the same universe rule in validation and live.
  momUniverseSize: 1200,       // [A] most liquid names by median 20-session dollar volume
  momMinBars: 760,
};
...
// Rules are fixed before their first validation run. Adding rules, or tuning these numbers after
...
export function prepare(bars, cfg = CONFIG) {
  ...
  const high252 = new Array(bars.length).fill(NaN);
  for (let i = 251; i < bars.length; i++) { let m = -Infinity; for (let k = i - 251; k <= i; k++) if (bars[k].h > m) m = bars[k].h; high252[i] = m; }
  const mom = c.map((_, i) => i >= 252 ? c[i - 21] / c[i - 252] - 1 : NaN);   // 12-1 month momentum
  return { bars, c, sma5: rollingMean(c, 5), sma10: rollingMean(c, 10), sma20: rollingMean(c, 20), sma50: rollingMean(c, 50), sma200: rollingMean(c, 200),
    rsi: rsiSeries(c, 14), rsi2: rsiSeries(c, 2), atr: atrSeries(bars, cfg.atrPeriod), dv: bars.map(b => b.c * b.v), vol: bars.map(b => b.v), high20, high252, mom,
    momPct: new Array(bars.length).fill(NaN) };   // filled by attachMomentumRanks (cross-sectional)
}
// The ranked cross-section: the momUniverseSize most liquid names with momMinBars of history.
// validate.js and daily.js both call this so a "top decile" name means the same thing in each.
export function momentumUniverse(preparedBySymbol, cfg = CONFIG) {
  const dv = s => median(s.dv.slice(-20));
  return Object.fromEntries(Object.entries(preparedBySymbol).sort((a, b) => dv(b[1]) - dv(a[1])).slice(0, cfg.momUniverseSize).filter(([, s]) => s.bars.length >= cfg.momMinBars));
}
// Cross-sectional 12-1 momentum percentile per date, across whatever universe is passed in.
export function attachMomentumRanks(preparedBySymbol) {
  const byDate = new Map();
  for (const s of Object.values(preparedBySymbol)) s.bars.forEach((b, i) => { if (Number.isFinite(s.mom[i])) { const d = b.t.slice(0, 10); if (!byDate.has(d)) byDate.set(d, []); byDate.get(d).push([s, i, s.mom[i]]); } });
  for (const rows of byDate.values()) {
    if (rows.length < 50) continue;   // [A] too few names that day for a meaningful rank
    rows.sort((a, b) => a[2] - b[2]);
    rows.forEach(([s, i], k) => { s.momPct[i] = k / (rows.length - 1); });
  }
}
...
export const SETUPS = {
  ...
  // --- Added Sept 25, 2026, written down BEFORE their first validation run. ---
  // News/earnings gap that holds: big gap up on heavy volume that closes strong. Post-event drift.
  gapdrift: {
    family: 'trend', label: 'Volume gap that holds (≤20 sessions)',
    at(s, i, cfg = CONFIG) {
      const p = basePlan(s, i, cfg); if (!p) return null;
      const b = s.bars[i], prev = s.bars[i - 1], range = b.h - b.l;
      if (!(b.o >= prev.c * 1.03 && b.c >= b.o && range > 0 && (b.c - b.l) / range >= 0.5)) return null;
      if (!(b.v >= 3 * median(s.vol.slice(i - 20, i)) && b.c > s.sma50[i])) return null;
      return finish(p, Math.min(b.l - 0.1 * p.atr, p.limit - 1.5 * p.atr), null, 20, cfg);
    },
  },
  // First new 52-week high after a month without one, in an established uptrend (George & Hwang).
  high52: {
    family: 'trend', label: 'Fresh 52-week high (≤20 sessions)',
    at(s, i, cfg = CONFIG) {
      const p = basePlan(s, i, cfg); if (!p || i < 280) return null;
      if (!(s.bars[i].h >= s.high252[i] && s.high252[i - 1] > Math.max(...s.bars.slice(i - 21, i).map(x => x.h)))) return null;
      if (!(p.close > s.sma50[i] && s.sma50[i] > s.sma200[i])) return null;
      return finish(p, p.limit - 3 * p.atr, null, 20, cfg);
    },
  },
  // Top-decile 12-1 momentum names on a short pullback (Jegadeesh–Titman leaders, bought on weakness).
  leaderdip: {
    family: 'trend', label: 'Momentum leader pullback (≤10 sessions)',
    at(s, i, cfg = CONFIG) {
      const p = basePlan(s, i, cfg); if (!p) return null;
      if (!(s.momPct[i] >= 0.9 && p.close < s.sma10[i] && p.close > s.sma50[i])) return null;
      return finish(p, p.limit - 2.5 * p.atr, null, 10, cfg);
    },
  },
...
export function cellStatus(validation, setupKey, label, now = new Date(), cfg = CONFIG) {
  ...
  const tier = cellTier(cell, cfg);
  return { enabled: !!tier, tier, why: tier === 'full' ? 'validated' : tier === 'probation' ? `probation (${cell.fails.join('; ')})` : cell.fails.join('; '), cell };
}
// 'full' | 'probation' | null, from the stored fold + pooled stats (works on older validation files).
export function cellTier(cell, cfg = CONFIG) {
  if (!cell) return null;
  if (cell.pass) return 'full';
  if (!(cfg.probationRiskScale > 0)) return null;
  const folds = cell.folds || [], p = cell.pooled || {};
  const ok = folds.length >= 2 && folds.every(f => f.n >= cfg.cellMinTrades && f.avgR > cfg.probationMinAvgR) && p.pf >= cfg.probationMinPF && p.t >= cfg.probationMinT;
  return ok ? 'probation' : null;
}
export function judgeCell(folds, cfg = CONFIG) {
  ...
  const res = { pass: !fails.length, fails, pooled: all };
  return { ...res, tier: cellTier({ ...res, folds }, cfg) };
}
...
export function readSymbol({ symbol, bars, spy, regime, validation, cfg = CONFIG, equity, riskPct, now = new Date(), prepared }) {
  ...
  const s = prepared || prepare(bars, cfg), i = bars.length - 1;
  ...
  for (const [key, st, plan] of fired) {
    ...
    r.candidates.push({ key, label: st.label, plan, cell: cell.cell, tier: cell.tier });
  }
  ...
  r.candidates.sort((a, b) => (b.tier === 'full') - (a.tier === 'full') || b.cell.pooled.t - a.cell.pooled.t);
  const best = r.candidates[0], plan = best.plan, tierScale = best.tier === 'probation' ? cfg.probationRiskScale : 1;
  ...
  const wanted = riskPct * regime.riskScale * tierScale, riskPctUsed = Math.min(wanted, cfg.maxTradeRisk);
  if (wanted > riskPctUsed) r.notes.push(`risk capped at the ${cfg.maxTradeRisk * 100}% per-trade ceiling`);
  const riskDollars = equity * riskPctUsed;
  ...
  const posCap = Math.floor(cfg.maxPositionPct * equity / plan.limit);
  if (shares > posCap) { shares = posCap; r.notes.push(`size capped at ${cfg.maxPositionPct * 100}% of the account (tight stop)`); }
  r.sizing = { riskPct: riskPctUsed, riskDollars, shares, notional: shares * plan.limit };
  r.tier = best.tier; r.rank = best.cell.pooled.t;
  // Plain facts about the stock for the message (display only, never a gate).
  const c = s.c, yr = bars.slice(-252);
  r.profile = {
    close: c[i], changePct: c[i] / c[i - 1] - 1, sma20: s.sma20[i], sma50: s.sma50[i], sma200: s.sma200[i],
    high52: Math.max(...yr.map(b => b.h)), low52: Math.min(...yr.map(b => b.l)),
    rsi: s.rsi[i], rsi2: s.rsi2[i], atrPct: s.atr[i] / c[i], momPct: s.momPct[i], high20: s.high20[i],
    gapPct: bars[i].o / c[i - 1] - 1, volMult: bars[i].v / median(s.vol.slice(i - 20, i)),
    ret6m: c[i] / c[i - 126] - 1, spyRet6m: spy.length > 126 ? spy.at(-1).c / spy.at(-127).c - 1 : NaN,
  };
  return r;
}
